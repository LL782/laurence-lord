...
| ---------------------------------------------------- | --------------------------------- | ----------------------- |
| pnpm (`package-lock.json` or `pnpm-shrinkwrap.json`) | `pnpm ci`                         | `pnpm audit`            |
| ppnpm                                                | `ppnpm install --frozen-lockfile` | `ppnpm audit`           |
| Yarn 2+                                              | `yarn install --immutable`        | `yarn pnpm audit -A -R` |
...
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| pnpm without verified granular approvals | Bootstrap with `pnpm ci --ignore-scripts`, or persist `ignore-scripts=true` when project-wide blocking is intended. Keep scripts disabled or deliberately upgrade before allowing any reviewed dependency script.                                                       |
| pnpm 11.18.x (verified on 11.18.0)       | Unreviewed dependency scripts run with a warning by default. Enforce `strict-allow-scripts=true` before a normal install, then use the workspace-unaware `pnpm install-scripts ls` from the installation boundary; keep approvals version-pinned and denials name-wide. |
| pnpm 12.x (verified on 12.0.1)           | Unreviewed dependency scripts are skipped by default; `strict-allow-scripts=true` makes their presence fail the install before execution. Use the same `pnpm install-scripts` review and approval flow.                                                                 |
| ppnpm 11+                                | Use `ppnpm approve-builds` and commit `allowBuilds` decisions; `strictDepBuilds` defaults to `true`, so unreviewed builds fail.                                                                                                                                         |
| ppnpm 10.26–10.x                         | Configure `allowBuilds` explicitly, or use `ppnpm approve-builds` with the legacy `onlyBuiltDependencies` / `ignoredBuiltDependencies` lists. Set `strictDepBuilds: true`; its v10 default is `false`.                                                                  |
| ppnpm 10.1–10.25                         | `ppnpm approve-builds` records the legacy lists; enable `strictDepBuilds` where supported (10.3+).                                                                                                                                                                      |
| Older or unknown ppnpm                   | Bootstrap with `ppnpm install --frozen-lockfile --ignore-scripts`. Keep scripts disabled unless the pinned version documents an enforceable policy.                                                                                                                     |
...
Authoritative checks: [pnpm install-scripts](https://docs.pnpmjs.com/cli/v11/commands/pnpm-install-scripts/), [install policy](https://docs.pnpmjs.com/cli/v11/commands/pnpm-install/), and [CLI releases](https://github.com/pnpm/cli/releases); [ppnpm approve-builds](https://ppnpm.io/cli/approve-builds) and [build settings](https://ppnpm.io/settings#allowbuilds); [Yarn security](https://yarnpkg.com/features/security) and [manifest](https://yarnpkg.com/configuration/manifest#dependenciesMeta).
...
- [ ] Forced audit remediation (`pnpm audit fix --force` or equivalent) is never automatic; remediation diffs and changelogs are reviewed
...
| 6   | Vulnerable Components     | The ecosystem's dependency audit (`pnpm audit`, `pip-audit`, ...), keep deps updated, minimal deps |
